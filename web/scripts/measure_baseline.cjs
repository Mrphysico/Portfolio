...
async function measure() {
  console.log('=== STARTING PRODUCTION BASELINE MEASUREMENT (PORT 4173) ===');
  ...
  let bundleStats = { jsFiles: [], cssFiles: [], totalJsBytes: 0, totalCssBytes: 0 };
  ...
  console.log('Bundle Stats:', {
    totalJsKb: (bundleStats.totalJsBytes / 1024).toFixed(2),
    totalCssKb: (bundleStats.totalCssBytes / 1024).toFixed(2),
    jsFiles: bundleStats.jsFiles,
  });
  ...
  // ================= SCENARIO 1: DESKTOP BASELINE & SCENARIOS =================
  ...
  const tracePath = path.join(ARTIFACT_DIR, 'baseline_trace.zip');
  await context.tracing.stop({ path: tracePath });
  console.log(`Saved trace to: ${tracePath}`);

  // ================= SCENARIO 6: MOBILE BASELINE =================
  console.log('Measuring Mobile Baseline (Viewport 393x852)...');
  ...
  const reportPath = path.join(ARTIFACT_DIR, 'baseline_report.json');
  fs.writeFileSync(reportPath, JSON.stringify(baselineReport, null, 2));
  console.log(`Saved baseline report to: ${reportPath}`);
